...
router.post("/get-entregas", auth, async (req, res) => {
    try {
        ...
        if (user.tipoUsuario !== "profesor") {
            return res.status(403).json({ message: "Acceso denegado. Solo los profesores pueden ver las entregas." });
        }

        const entregas = await entregaService.getEntregasPorTarea(req.body.tareaId);
        res.json(entregas);
